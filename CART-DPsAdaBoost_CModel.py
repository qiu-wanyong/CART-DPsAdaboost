...
from read_data_adult import dt_data, predict_data
...
class CartAdaBoostAdult:
    ...
    def AdaBoost_adult(self):
        ...
        return pre_correct, plot_xy, area


data_dict = OrderedDict()

# 树的深度列表
tree_depth_list = list(range(2, 7, 1))

correct_dict = OrderedDict({})

correct_dict['CART'] = []
correct_dict['AdaBoost'] = []

# ROC 数据
roc_correct_dict = OrderedDict({})

roc_correct_dict['AdaBoost'] = []

# 面积
area_correct_dict = OrderedDict({})

area_correct_dict['AdaBoost'] = []

# CART模型
for tree_depth in tree_depth_list:
    # CART模型, modelcount, e可取任意的值
    cart_model = CartAdaBoostAdult(tree_depth, 3, 0.2, 'f')
    print('模型CART：树的深度%d' % cart_model.tree_length)
    c = cart_model.AdaBoost_adult()
    correct_dict['CART'].append(c)
    print('准确率: ', c)
...
for tree_depth in tree_depth_list:
    # CARTAdaBoost模型
    CARTAdaBoost_model = CartAdaBoostAdult(tree_depth, 10, 0.2, 'a')
    print('模型CARTAdaBoost：树的深度%d,基分类器个数%d' % (CARTAdaBoost_model.tree_length, CARTAdaBoost_model.model_c))
    c, plotxt, area = CARTAdaBoost_model.AdaBoost_adult()
    correct_dict['AdaBoost'].append(c)
    if tree_depth == 5:
        roc_correct_dict['AdaBoost'] = plotxt
        area_correct_dict['AdaBoost'] = area
    print('准确率: ', c)

print(correct_dict)
print(roc_correct_dict)
print(area_correct_dict)
